...
describe("dataURLToBytes", () => {
  ...
  test("should correctly extract MIME type and bytes from an SVG data URL", () => {
    const dataURL = "data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSIxMDAiIGhlaWdodD0iMTAwIj48Y2lyY2xlIGN4PSI1MCIgY3k9IjUwIiByPSI0MCIgc3Ryb2tlPSJibGFjayIgc3Ryb2tlLXdpZHRoPSIzIiBmaWxsPSJyZWQiIC8+PC9zdmc+";
    const { mime, bytes } = dataURLToBytes(dataURL);
  ...
  test("should handle data URLs without a MIME type", () => {
    const dataURL = "data:;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7";
    const { mime, bytes } = dataURLToBytes(dataURL);
